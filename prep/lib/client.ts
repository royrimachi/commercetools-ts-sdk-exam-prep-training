...
const createApiClient = (prefix: Prefix) => {
  const { clientId, clientSecret, host, oauthHost, projectKey }: Config =
    readConfig(prefix);
...
const createImportApiClient = (prefix: Prefix) => {
  const { clientId, clientSecret, oauthHost, projectKey }: Config =
    readConfig(prefix);
...
export const apiRoot: ApiRoot = createApiClient(Prefix.PREP);
export const importApiRoot: ImportApiRoot = createImportApiClient(Prefix.PREP);
